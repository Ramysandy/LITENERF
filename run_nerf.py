...
def render(H, W, focal,
    ...
    if use_viewdirs:
        ...
        if c2w_staticcam is not None:
            ...
            rays_o, rays_d = generate_rays(H, W, focal, c2w_staticcam)
...
def create_nerf(args):
    ...
    embed_fn, input_ch = build_positional_encoder(args.multires, args.i_embed)

    input_ch_views = 0
    embeddirs_fn = None
    if args.use_viewdirs:
        embeddirs_fn, input_ch_views = build_positional_encoder(
...
def train():
    ...
    if args.render_only:
        ...
        imageio.mimwrite(os.path.join(testsavedir, 'video.mp4'),
                         convert_to_uint8(rgbs), fps=30, quality=8)
    ...
    if use_batching:
        ...
        print('get rays')
        # generate_rays_np() returns rays_origin=[H, W, 3], rays_direction=[H, W, 3]
        # for each pixel in the image. This stack() adds a new dimension.
        rays = [generate_rays_np(H, W, focal, p) for p in poses[:, :3, :4]]
    ...
    for i in range(start, N_iters):
        ...
        with tf.GradientTape() as tape:
            ...
            img_loss = compute_img_mse(rgb, target_s)
            trans = extras['raw'][..., -1]
            loss = img_loss
            ...
            if 'rgb0' in extras:
                img_loss0 = compute_img_mse(extras['rgb0'], target_s)
                loss += img_loss0
                psnr0 = mse_to_psnr(img_loss0)
        ...
        if i % args.i_video == 0 and i > 0:
            ...
            imageio.mimwrite(moviebase + 'disp.mp4',
                             convert_to_uint8(disps / np.max(disps)), fps=30, quality=8)
            ...
            if args.use_viewdirs:
                ...
                imageio.mimwrite(moviebase + 'rgb_still.mp4',
                                 convert_to_uint8(rgbs_still), fps=30, quality=8)
        ...
        if i % args.i_print == 0 or i < 10:
            ...
            if i % args.i_img == 0:
                ...
                psnr = mse_to_psnr(compute_img_mse(rgb, target))
                
                # Save out the validation image for Tensorboard-free monitoring
                testimgdir = os.path.join(basedir, expname, 'tboard_val_imgs')
                if i == 0:
                    os.makedirs(testimgdir, exist_ok=True)
                imageio.imwrite(os.path.join(testimgdir, '{:06d}.png'.format(i)), convert_to_uint8(rgb))

                with tf.contrib.summary.record_summaries_every_n_global_steps(args.i_img):

                    tf.contrib.summary.image('rgb', convert_to_uint8(rgb)[tf.newaxis])
                ...
                if args.N_importance > 0:
                    ...
                    with tf.contrib.summary.record_summaries_every_n_global_steps(args.i_img):
                        tf.contrib.summary.image(
                            'rgb0', convert_to_uint8(extras['rgb0'])[tf.newaxis])
                        tf.contrib.summary.image(
